...
namespace RecyclableStream
{
    ...
    public partial class RecyclableMemoryStreamManager<T>
    {
        /// <summary>
        ...
        /// </summary>
        private readonly ConcurrentStack<T[]>[] largePools;

        private readonly int maximumBufferSize;
        private readonly ConcurrentStack<T[]> smallPool;
        ...
        public RecyclableMemoryStreamManager(int blockSize, int largeBufferMultiple, int maximumBufferSize)
        {
            ...
            this.smallPool = new ConcurrentStack<T[]>();
            ...
            this.largePools = new ConcurrentStack<T[]>[numLargePools];

            for (var i = 0; i < this.largePools.Length; ++i)
            {
                this.largePools[i] = new ConcurrentStack<T[]>();
            }
        ...
        internal T[] GetLargeBuffer(int requiredSize, string tag)
        {
        ...
        internal void ReturnLargeBuffer(T[] buffer, string tag)
        {
            if (buffer == null)
            {
        ...
        internal void ReturnBlocks(ICollection<T[]> blocks, string tag)
        {
            if (blocks == null)
            {
        ...
        public MemoryStream<T> GetStream()
        {
            return new RecyclableMemoryStream<T>(this);
        }
        ...
        /// <returns>A MemoryStream.</returns>
        public MemoryStream<T> GetStream(string tag)
        {
            return new RecyclableMemoryStream<T>(this, tag);
        }
        ...
        /// <returns>A MemoryStream.</returns>
        public MemoryStream<T> GetStream(string tag, int requiredSize)
        {
            return new RecyclableMemoryStream<T>(this, tag, requiredSize);
        }
        ...
        /// <returns>A MemoryStream.</returns>
        public MemoryStream<T> GetStream(string tag, int requiredSize, bool asContiguousBuffer)
        {
            ...
            return new RecyclableMemoryStream<T>(this, tag, requiredSize, this.GetLargeBuffer(requiredSize, tag));
        }
        ...
        public MemoryStream<T> GetStream(string tag, T[] buffer, int offset, int count)
        {
            var stream = new RecyclableMemoryStream<T>(this, tag, count);
